...
def apply_traffic(world, traffic_manager, num_vehicles, num_pedestrians, safe=True):
    # --------------
    # Spawn vehicles
    # --------------
    blueprints = world.get_blueprint_library().filter("vehicle.*")
    if safe:
        blueprints = list(filter(lambda x: int(x.get_attribute('number_of_wheels')) == 4 and
                (x.id.endswith('microlino') or
                 ...
                 x.id.endswith('ambulance') or
                 x.id.endswith('fusorosa')), blueprints))
